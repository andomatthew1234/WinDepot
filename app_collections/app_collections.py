...
COLLECTIONS_DATA = {
    ...
    "🎮 Gaming Essentials": {
        "description": "Everything you need to deploy gaming frameworks, storefronts, and chat clients.",
        "apps": [
...
class CollectionsManager:
    ...
    def open_collection(self, collection_name):
        ...
        apps = COLLECTIONS_DATA[collection_name]["apps"]
        for app in apps:
    ...
    def save_bulk_items(self, items):
        try:
            ...
            messagebox.showerror("Error", f"Failed to save bulk list: {e}")
    ...
    def add_single_to_bulk(self):
        ...
        if any(i["id"] == app_id for i in items):
            messagebox.showinfo("Already Added", f"'{app_name}' is already in your bulk list.")
        else:
            items.append({"name": app_name, "id": app_id})
            self.save_bulk_items(items)
            messagebox.showinfo("Success", f"Added '{app_name}' to the bulk queue!\nYou can start the bulk installer from the main WinDepot homepage.")

    def add_all_to_bulk(self):
        apps = COLLECTIONS_DATA[self.current_collection]["apps"]
        items = self.get_bulk_items()
        added_count = 0
        ...
        if added_count > 0:
            self.save_bulk_items(items)
            messagebox.showinfo("Success", f"Added {added_count} apps to your bulk queue!")
        else:
            messagebox.showinfo("Notice", "All apps in this collection are already in your bulk queue.")
        ...
        def worker():
            try:
                ...
                if process.returncode == 0 or process.returncode == 2316632107:
                    self.root.after(0, lambda: messagebox.showinfo("Installed", f"Successfully installed {app_name}!"))
                else:
                    self.root.after(0, lambda: messagebox.showwarning("Warning", f"Installation for {app_name} ended with code {process.returncode}."))
            except Exception as e:
                self.root.after(0, lambda err=e: messagebox.showerror("Error", f"Failed to install: {err}"))
            finally:
